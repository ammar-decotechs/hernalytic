...
export default function Table({initialData ,tableDataKey, activityHeaderRows}:any) {
  ...
  const handleCheckChange = (index: number) => {
    const updatedUsers = users.map((user, i) => {
      if (i === index) {
        return { ...user, check: !user.check }; // Toggle boolean value
      }
      return user;
    });
  ...
  return (
    ...
    <div className="pt-4 pb-8 border-t mt-10 border-r border-l border-primary-cLightF1 rounded-[8px] no_scrollbar overflow-x-auto">
      <div className="lg:w-full w-[1100px]">
        ...
        <div className="grid grid-cols-11 px-6 text-primary-cDark1D bg-primary-theadingBgFD text-[16px] font-[400] py-4">
          <div className="col-span-1">
            <div className="w-4 h-4 p-2 rounded-[2px] flex justify-center items-center border-2 border-primary-cDark1D cursor-pointer"
            >-</div>
          </div>
          <div className="col-span-5">Name</div>
          <div className="col-span-2 w-full flex justify-center">Role</div>
          <div className="col-span-2 w-full text-end">Create Date</div>
          <div className="col-span-1 w-full flex justify-center">Active</div>
        </div>

        {users.map((user, index) => (
          <div key={index} className="grid px-6 grid-cols-11 items-center text-primary-cDark1D text-[16px] font-[400] py-4">
            <div className="col-span-1">
              {/* Custom checkbox */}
              <div
