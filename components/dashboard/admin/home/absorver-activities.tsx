...
import { getAdminObserverActivities } from "@/redux/services/admin/admin";
import { useEffect } from "react";
import { obserActivities } from "@/redux/interface/admin";

export default function AbsorverActivities({setSeeall}:any) {
  const {adminObserver} = useAppSelector(state=>state.admin)

  // const data = [
  //   {
  //     observer: "Adeyemi Ojo",
  //     SurvaySubmitted: 2,
  //     SurvayNotSubmitted: 3,
  //   },
  //   {
  //     observer: "Stephine Okeowo",
  //     SurvaySubmitted: 3,
  //     SurvayNotSubmitted: 4,
  //   },
  //   {
  //     observer: "Adeyemi Ojo",
  //     SurvaySubmitted: 10,
  //     SurvayNotSubmitted: 5,
  //   },
  //   {
  //     observer: "Adeyemi Ojo",
  //     SurvaySubmitted: 20,
  //     SurvayNotSubmitted: 33,
  //   },
  // ];
  ...
  useEffect(()=>{
    (async()=>{
      try {
        const res = await getAdminObserverActivities();
        console.log("--",res);
      } catch (error) {
  ...
  return (
    <div className="px-6 pt-4 pb-8 border-t  border-r border-l border-primary-cLightF1 bg-primary-cLightbgFD rounded-[8px] rounded-tr-[8px]  overflow-x-auto no_scrollbar">
        ...
        {
          adminObserver.length>3 && 
          <div className="flex items-center cursor-pointer" onClick={()=>setSeeall("observer")}>
          <p className="text-[16px] text-primary-cOrange00">See All</p>
        </div>
        }
      ...
      {adminObserver?.slice(0,3).map((data:obserActivities) => (
