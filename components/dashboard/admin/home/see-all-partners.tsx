import { useAppSelector } from '@/redux/hooks';
import React from 'react'
import { partnerDonwload } from "@/redux/interface/admin";


export default function SeeAllPartners() {

  const { PartnerDownload } = useAppSelector(state => state.admin);

  // const data = [
  //     {
  //       Partners: "Adeyemi Ojo",
  //       SurvaySubmitted: 100,
  //       SurvayDownloaded: 100,
  //     },
  //     {
  //       Partners: "Stephine Okeowo",
  //       SurvaySubmitted: 20,
  //       SurvayDownloaded: 20,
  //     },
  //     {
  //       Partners: "Adeyemi Ojo",
  //       SurvaySubmitted: 40,
  //       SurvayDownloaded: 40,
  //     },
  //     {
  //       Partners: "Adeyemi Ojo",
  //       SurvaySubmitted: 40,
  //       SurvayDownloaded: 40,
  //     },
  //     {
  //       Partners: "Adeyemi Ojo",
  //       SurvaySubmitted: 40,
  //       SurvayDownloaded: 40,
  //     },
  //     {
  //       Partners: "Adeyemi Ojo",
  //       SurvaySubmitted: 40,
  //       SurvayDownloaded: 40,
  //     },
  //     {
  //       Partners: "Adeyemi Ojo",
  //       SurvaySubmitted: 40,
  //       SurvayDownloaded: 40,
  //     },
  //     {
  //       Partners: "Adeyemi Ojo",
  //       SurvaySubmitted: 40,
  //       SurvayDownloaded: 40,
  //     },
  //     {
  //       Partners: "Adeyemi Ojo",
  //       SurvaySubmitted: 40,
  //       SurvayDownloaded: 40,
  //     },
  //     {
  //       Partners: "Adeyemi Ojo",
  //       SurvaySubmitted: 40,
  //       SurvayDownloaded: 40,
  //     },
  //     {
  //       Partners: "Adeyemi Ojo",
  //       SurvaySubmitted: 40,
  //       SurvayDownloaded: 40,
  //     },
  //     {
  //       Partners: "Adeyemi Ojo",
  //       SurvaySubmitted: 40,
  //       SurvayDownloaded: 40,
  //     },

  //   ];
  return (
    <div className="px-6 pt-4 pb-8 border-t  border-r border-l border-primary-cLightF1 bg-primary-cLightbgFD rounded-[8px] rounded-tr-[8px]  overflow-x-auto no_scrollbar mt-28">
      <div className="sm:w-full w-[600px]">
        <div className="flex items-center py-2 justify-between border-b border-primary-cLightF1">
          <p className="text-[24px] text-primary-cDark1D font-semibold">
            Partners Activities
          </p>
          <div className="flex items-center">
            <p className="text-[16px] text-primary-cOrange00  hidden">See All</p>
          </div>
        </div>

        <div className="grid grid-cols-7  gap-x-16 text-primary-cDark1D text-[16px] font-[600] mt-4">
          <div className="col-span-3">Partners</div>
          <div className="col-span-2 w-full text-end">Survay Submitted</div>
          <div className="col-span-2  text-end w-fulls">Survey Downloaded</div>
        </div>
        {PartnerDownload?.length > 0 && PartnerDownload?.map((data: partnerDonwload) => (
          <div key={data.partner_id} className="grid grid-cols-6 items-center    text-primary-cDark1D text-[16px] font-[500] mt-4 ">
            <div className="col-span-3 flex items-center gap-2">
              <div className="w-12 h-12  overflow-hidden rounded-full border">
                <img
                  src="data:image/png;base64,"
                  alt=""
                  className="w-full "
                />
              </div>
              <p className=" text-primary-cDark1D ">{data.firstname} {data.lastname}</p>
            </div>
            <div className="col-span-3 w-full text-end">
              {data.survey_downloaded}
            </div>

          </div>
        ))}
      </div>

    </div>
  )
}


