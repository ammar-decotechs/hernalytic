import { getPartnerDownload } from "@/redux/services/admin/admin";
import React, { useEffect } from "react";
import { useAppSelector } from "@/redux/hooks";
import { partnerDonwload } from "@/redux/interface/admin";

export default function PartnersActivities({ setSeeall }: any) {

  const { PartnerDownload } = useAppSelector(state => state.admin);


  useEffect(() => {
    (async () => {
      try {
        const res = await getPartnerDownload();

      } catch (error) {
        console.log("error", error);
      }
    })()
  }, [])

  return (
    <div className="px-6 pt-4 pb-8 border-t  border-r border-l border-primary-cLightF1 bg-primary-cLightbgFD rounded-[8px] w-full overflow-x-auto no_scrollbar ">
      <div className="sm:w-full w-[600px]">
        <div className="flex items-center py-2 justify-between border-b border-primary-cLightF1">
          <p className="text-[24px] text-primary-cDark1D font-semibold">
            Partners Activities
          </p>
          <div className="flex items-center" onClick={() => setSeeall("partner")}>
            {
              PartnerDownload.length > 3 &&
              <p className="text-[16px] cursor-pointer text-primary-cOrange00">See All</p>
            }
          </div>
        </div>

        <div className="grid grid-cols-6   text-primary-cDark1D text-[16px] font-[600] mt-4 ">
          <div className="col-span-3">Partner</div>
          <div className="col-span-3 w-full text-end">Survey Downloaded</div>
        </div>
        {PartnerDownload.slice(0, 3).map((data: partnerDonwload) => (
          <div key={data.partner_id} className="grid grid-cols-6 items-center    text-primary-cDark1D text-[16px] font-[500] mt-4 ">
            <div className="col-span-3 flex items-center gap-2">
              <div className="w-12 h-12  overflow-hidden rounded-full border">
                <img
                  src="data:image/png;base64,"
                  alt=""
                  className="w-full "
                />
              </div>
              <p className=" text-primary-cDark1D ">{data.firstname} {data.lastname}</p>
            </div>
            <div className="col-span-3 w-full text-end">
              {data.survey_downloaded}
            </div>

          </div>
        ))}
      </div>

    </div>
  );
}
